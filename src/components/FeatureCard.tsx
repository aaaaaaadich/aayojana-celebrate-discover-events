...
const FeatureCard = ({ title, description, icon: Icon, iconColor = "#F4A261" }: FeatureCardProps) => {
  return (
    <Card className="bg-card transition-all duration-500 hover:shadow-2xl h-full animate-hover-lift animate-3d-hover glow-on-hover animate-morph group overflow-hidden">
      ...
      <CardContent className="p-6 flex flex-col items-center text-center relative z-10">
        <div className="h-14 w-14 rounded-full bg-saffron-100 dark:bg-saffron-900/30 flex items-center justify-center mb-4 transition-all duration-500 group-hover:scale-110 animate-3d-hover glow-on-hover animate-magnetic">
          {/* Icon Glow Effect */}
          ...
          <Icon 
            size={28} 
            color={iconColor} 
            className="transition-all duration-500 group-hover:rotate-12 animate-elastic relative z-10" 
          />
        </div>
        
        <h3 className="text-xl font-bold mb-2 transition-all duration-500 group-hover:text-saffron-500 animate-shimmer">
          {title}
        </h3>
